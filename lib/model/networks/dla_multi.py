...
class DLA(nn.Module):
    ...
    def forward(self, x, pre_img=None, pre_hm=None):
        ...
        if pre_img is not None:
            for i in range(pre_img.shape[1]):
                # pre_img_layer = getattr(self, 'pre_img_layer_' + str(i))
                # x = x + pre_img_layer(pre_img[:, i, ...])
                x = x + self.pre_img_layer(pre_img[:, i, ...])
        if pre_hm is not None:
            for i in range(pre_hm.shape[1]):
                # pre_hm_layer = getattr(self, 'pre_hm_layer_' + str(i))
                # x = x + pre_hm_layer(pre_hm[:, i, ...])
                x = x + self.pre_hm_layer(pre_hm[:, i, ...])
        for i in range(6):
