...
class TPSBendLoss(nn.Module):
    def __init__(self):
        super(TPSBendLoss, self).__init__()

    def u(self, r):
        return r**2 * torch.log(r)

    def d(self, a, b):
        return torch.sqrt(torch.square(a[:, None, :2] - b[None, :, :2]).sum(-1) + 1e-6)

    def fit(self, c, lambd=1.):
        n = c.shape[0]
        U = self.u(self.d(c, c))
        K = U + torch.eye(n, dtype=torch.float64).cuda() * lambd
        P = torch.ones((n, 3), dtype=torch.float64).cuda()
        P[:, 1:] = c[:, :2]
        L = torch.zeros((n + 3, n + 3), dtype=torch.float64).cuda()
        L[:n, :n] = K
        L[:n, -3:] = P
        L[-3:, :n] = P.T
        L_1 = torch.inverse(L)
    ...
    def forward(self, c_src, c_dst):
        num = c_src.shape[0]
        loss = 0
        for i in range(num):
            H = self.fit(c_src[i])
            x, y = c_dst[i, :, [0]], c_dst[i, :, [1]]
            x_ = torch.mm(torch.mm(x.T, H), x)
            y_ = torch.mm(torch.mm(y.T, H), y)
            loss += max((x_[0, 0] + y_[0, 0]) / math.pi / 8, 1e-6)
        return loss / num
