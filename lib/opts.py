...
class opts(object):
  ...
  def parse(self, args=''):
    ...
    # log dirs
    opt.root_dir = os.path.join('/public2/liangzhichao/ContiSeg/')
    opt.data_dir = os.path.join(opt.root_dir, 'data', opt.task)
    opt.exp_dir = os.path.join(opt.root_dir, 'exp/checkpoints')
    opt.save_dir = os.path.join(opt.exp_dir, opt.input_mode)
    opt.debug_dir = os.path.join(opt.save_dir, 'debug')
    
    if opt.resume and opt.load_model == '':
      opt.load_model = os.path.join(opt.save_dir, 'model_99.pth')
    return opt
